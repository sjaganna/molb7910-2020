...
Please execute the following tasks, knit the resulting .Rmd (`Lastname_class5-homework.Rmd`) and submit the .html output as your assignment.
...
#### 1. Load packages 
Packages: `tidyverse`, `ggplot2`, `edgeR`
```{r, include = TRUE}
#load packages here
...
#### 2. (Optional) Set working directory, if need be
```{r}
#setwd/here
...
#### 3. Import data
Path: "5-class4/data/input/data_gene_level.csv"
```{r}
#import data
...
Follow the same steps 1-10 we used for the 14h sample in our class. Note that the only changes you will make to this pipeline is to subset the geneExpr dataframe differently (step #1) and to save the final foldchange data with the appropriate name `(step #10). However, please look up the steps in edgeR (use the references at the bottom of `class5-DGE.Rmd`) and try to understand them. 
...
1. Move the rownames of `DUX4_8h$table` into a new column to enable the next steps.
